...
public class ChartView extends HBox {
    ...
    public ChartView(String[] objects, ArrayList<Double> comparative){
        ObservableList<PieChart.Data> pieChartData = FXCollections.observableArrayList(); // données observables
        for (int i = 0 ; i < (objects.length > 7 ? 7 : objects.length) ; i++ ){
            pieChartData.add(new PieChart.Data(objects[i], comparative.get(i)));
        }
        ...
        for (int i = 0 ; i < objects.length ; i++) {
            data.add(new ObservableEntity(objects[i], comparative.get(i)));
        }
